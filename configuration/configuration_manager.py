...
class Configuration(metaclass=Singleton):
	...
	def __init__(self, configuration):
		...
		# print(self.params['multiplexing'])
	...
	def _filter_by_key(self):
		...
		for key in list(self.initial_params.keys()):
			...
			if key in StageManager.get_all_available_run_stages():
				if key == RunStages.post_process.name or key == RunStages.pattern_generation.name or key == RunStages.plot_results.name or key == RunStages.multiplexing.name:
					for name, value in self.initial_params[key].items():
						self.initial_params[name] = value
	...
	def _add_common_calculated_values(self):
		...
		other parameters and shared between several modules
		:return:
		"""

		# get number of pixels in current design
