...
class PostProcessStage(CommonRunStage):
	...
	def _generate_potential_matrix_per_time_section(self, start_time, idx_time, total_time, average_over_pulse_duration=True):
		"""
		Currently this function is only used for the bipolar configuration and computing time slices. 
    TODO check in which commit average_over_pulse_duration disappeared, and make sure it is not an issue. 
    Especially in monopolar configuration. Nathan's notebook did not have it, slight conflict between Nathan and Anna's version.
		"""
	...
	def run_stage(self, *args, **kwargs):
      ...
      # TODO make sure that not having "on_diode_data": on_diode_data_during_stable_pulse
      # is not an issue for the monoppolar
		...
		if Configuration().params["model"] == Models.BIPOLAR.value:
			...
			for time_point_index, time_point_value in enumerate(self.time_points_to_analyze_ms):
				...
				voltage_3d_matrix = self._generate_potential_matrix_per_time_section(start_time=time_point_value, idx_time=time_point_index, total_time=len(self.time_points_to_analyze_ms),average_over_pulse_duration=self.average_over_pulse_duration)
