...
class CurrentSequenceStage(CommonRunStage):
    ...
    def __init__(self, *args):
        ...
        self.is_multiplexed = RunStages.multiplexing.name in self.outputs_container

        # determine whether to use masked images
        self.is_masked = RunStages.mask_projection.name in self.outputs_container
    ...
    def run_stage(self):
        ...
        for row in self.script:
            ...
            # Added rounding
            assert abs(round(sum(row_dat) - self.video_sequence['duration_frames_ms'],
                             6)) < 1e-6, "Frames must be of the same length!"  # TODO: change rounding error
        ...
        for frame_idx in range(len(self.script)):
            ...
            for sub_frame_idx in range(number_of_sub_frames):
                if self.is_generated or self.is_multiplexed:
                    ...
                    multiplex_found = True if len(glob.glob(
                        os.path.join(Configuration().params["user_input_path"], 'image_sequence',
                                     Configuration().params["video_sequence_name"],
                                     f"{self.video_sequence['frame_names'][frame_idx]}") + "/*multiplexed.bmp")) > 0 and self.use_multiplex else False
                    if multiplex_found:
        ...
        return [self.video_sequence, {"gif_data": self.gif_image, "gif_time": self.gif_time},
                self.determine_most_illuminated(), self.image_sequence_input_folder]
    ...
    def _parse_script_file(self):
        ...
        photocurrent = Configuration().params["photosensitive_area"] * Configuration().params[
            "light_to_current_conversion_rate"] \
